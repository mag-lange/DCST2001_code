...
class TaskService {
  ...
  create(title: string, description: string, done: boolean) {
    return new Promise<number>((resolve, reject) => {
      pool.query('INSERT INTO Tasks SET title=?, description=?, done=?', [title, description, done], (error, results: ResultSetHeader) => {
        if (error) return reject(error);
  ...
  update(id: number, title: string, description: string, done: boolean) {
    return new Promise<void>((resolve, reject) => {
      pool.query('UPDATE Tasks SET title=?, description=?, done=? WHERE id = ?', [title, description, done, id], (error, results: ResultSetHeader) => {
        if (error) return reject(error);
        if (results.affectedRows == 0) return reject(new Error('No was changed, something went wrong'));
