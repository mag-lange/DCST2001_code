      ...
      <>
        <Card title="Edit task">
          ...
          <Row>
            ...
            <Column>
              <Form.Textarea value={this.task.description} onChange={(event) => {this.task.description = event.currentTarget.value}} rows={10} />
            </Column>
          </Row>
          <Row>
            <Column width={2}>Done:</Column>
        ...
        <Row>
          <Column>
            <Button.Success onClick={() => {
              taskService.update(this.task.id, this.task.title, this.task.description, this.task.done)
              Alert.info("Task updated successfully")
              history.push("/tasks")
            }}>Save</Button.Success>
          </Column>
          <Column right>
            <Button.Danger onClick={() => {
              taskService.delete(this.task.id)
              Alert.info("Deletion successful")
              history.push('/tasks') //Here I return back to the start page
            }
              }
      ...
      <>
        <Card title="New task">
          ...
          <Row>
            ...
            <Column>
              <Form.Textarea value={this.description} onChange={(event) => (this.description) = event.currentTarget.value} rows={10}/>
            </Column>
          </Row>
        </Card>
        <Button.Success
          onClick={() => {
            taskService
              .create(this.title, this.description, false) //Since no task is done when it is created, I simply specify that false should be sent
              .then((id) => history.push('/tasks/' + id))
              .catch((error) => Alert.danger('Error creating task: ' + error.message));
              Alert.info("Task created succesfully")
          }}
        >
          Create
        </Button.Success>
